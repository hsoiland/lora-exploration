...
def apply_lora_rust(
    ...
    # Check if tensors are on GPU and we should use PyTorch instead of Rust
    if device.type == 'cuda':
        # Pure PyTorch implementation - no CPU transfers needed
        lora_delta = torch.matmul(lora_up.to(device), lora_down.to(device))
        updated_tensor = tensor + alpha * lora_delta.to(tensor.dtype)
        return updated_tensor
    
    # For CPU tensors, use the Rust backend
    ...
    try:
        ...
        # If Rust fails, fall back to PyTorch
...
def apply_lora_batch_rust(
    ...
    if device.type == 'cuda':
        ...
        for lora_down, lora_up, alpha in lora_pairs:
            lora_delta = torch.matmul(lora_up.to(device), lora_down.to(device))
            result += alpha * lora_delta.to(result.dtype)
        return result
    
    # For CPU tensors, use the Rust backend
    # Convert to CPU/float32 for Rust
    tensor_cpu = tensor.detach().cpu().to(torch.float32)
    ...
    try:
        ...
        print(f"⚠️ Rust batch implementation failed: {e}")
        print("Falling back to PyTorch implementation")
