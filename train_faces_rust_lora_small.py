...
class FaceDataset(Dataset):
    ...
    def __init__(self, image_dir, size=256, tokenizer=None):  # Reduced image size
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a small LoRA using face images with Rust backend")
    ...
    parser.add_argument("--output_dir", type=str, default="gina_rust_lora_small", 
                      help="Directory to save LoRA weights and samples")
    parser.add_argument("--lora_name", type=str, default="gina_faces_rust_lora_small", 
                      help="Name for the trained LoRA")
    parser.add_argument("--rank", type=int, default=4,  # Reduced rank for faster training
    ...
    parser.add_argument("--save_steps", type=int, default=10,  # Save more frequently
                      help="Save checkpoint every X steps")
    parser.add_argument("--seed", type=int, default=42, 
                      help="Random seed")
    parser.add_argument("--image_size", type=int, default=256,  # Reduced image size
...
# Select a small subset of important self-attention modules
def find_key_self_attention_modules(unet):
    """Find a small subset of key self-attention modules"""
    # Key middle and up blocks attention modules for faces
    key_module_patterns = [
    ...
    # Find all modules that match the key patterns
...
def initialize_lora_weights(unet, rank=4, target_modules=None):
    """Initialize LoRA weights for selected modules"""
    lora_weights = {}
...
def main():
    try:
        ...
        # Find key self-attention modules (smaller subset)
        target_modules = find_key_self_attention_modules(unet)
        print(f"Selected {len(target_modules)} key self-attention modules for training")
        
        # Create initial LoRA weights for the selected modules
        lora_weights = initialize_lora_weights(
            unet=unet,
            rank=args.rank,
        ...
        dataset = FaceDataset(
            ...
            size=args.image_size,  # Reduced image size
        ...
        try:
            ...
            apply_lora_to_self_attention(
                ...
                target_modules=target_modules,  # Only use our trained modules
            ...
            print("Your LoRA weights are still saved successfully at:")
            print(f"  {final_lora_path}")
            print("\nTo use these weights for inference, you'll need a complete SDXL model.")
